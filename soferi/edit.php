...
$pageTitle = 'Editează șofer';

$id = (int)($_GET['id'] ?? 0);
$stmt = $conn->prepare("SELECT * FROM soferi WHERE id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nume) {
        ...
        $stmt = $conn->prepare("UPDATE soferi SET nume=?, telefon=?, email=?, nr_permis=?, categorie_permis=?, data_expirare_permis=? WHERE id=?");
        $stmt->bind_param("ssssssi", $nume, $telefon, $email, $nr_permis, $categorie_permis, $data_expirare_permis, $id);
        $stmt->execute();
        $stmt->close();
    ...
    <div class="card-body">
      <form method="POST">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label>Data expirare permis</label>
